...
//    result.putExtra(SelectActivity.EX_PATH_RESULT, f.getAbsolutePath());
    result.putExtra(SelectActivity.EX_PATH_PARENT,f.getParent());
    result.putExtra(SelectActivity.EX_FILE_NAME,f.getName());
    activity.setResult(SelectActivity.RESULT_OK, result);
    Log.d("An.dt","duong dan "+f.getParent()+" ========= "+"ten file "+f.getName());
    activity.finish();
  }
